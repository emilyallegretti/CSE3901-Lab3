...
module Services
  ...
  class Parsing
    ...
    def self.load(campus_q, term_q)
      ...
      @pars_resp.each do |k|
        c = Course.create(name: k['course']['title'], number: k['course']['catalogNumber'],
                          term: k['course']['term'], campus: k['course']['campus'])
        ...
        sec.each do |e|
          s = Section.create(section_number: e['section'], start_time: e['meetings'][0]['startTime'],
                             end_time: e['meetings'][0]['endTime'], location: e['meetings'][0]['buildingDescription'], monday: e['meetings'][0]['monday'], tuesday: e['meetings'][0]['tuesday'], wednesday: e['meetings'][0]['wednesday'], thursday: e['meetings'][0]['thursday'], friday: e['meetings'][0]['friday'], saturday: e['meetings'][0]['saturday'], sunday: e['meetings'][0]['sunday'], mode_of_instruction: e['instructionMode'], course_id: c.id)
        end
      end
    end
  end
